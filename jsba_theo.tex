...
DDBS. Several advantages of distributed database systems are (cf. \citet{Jadhav2017} and \citet[pp.~8ff.]{Ozsu1991}):
...
infrastructure and hardware for a distribution to multiple sites come along with higher costs \citep{Kumar2013}.
...
database which yield the correct result set for the query in an optimal way (cf. \citet[p.~174]{Ozsu1991} and \citet{Sattler2009DistQuer}). However, under a 
...
\emph{Query decomposition} means the process of rewriting the query to a normalized form (cf. \citet[p.~21,pp.189f.]{Ozsu1991}), followed by a semantical
analysis, a simplification of the query and a rewriting and restructuring to a relational algebra query \citep[p.~183]{Ozsu1991}. These four steps provide
...
(cf. \citet[Figure~8.3, p.~195]{Ozsu1991}), and not to the SQL query directly. The actual distribution of the data does not play any role here because
...
global relations by reverting the fragmentation. This so-called \emph{localization program} \citep[p.~199]{Ozsu1991} obtained by reverting the 
...
the data. This \emph{reduction techniques} \citep[p.~199]{Ozsu1991} are dependent on the type of fragmentation, i.e. whether it is a (primary/derived)
...
execution from following rather bad strategies \citep[p.~210]{Ozsu1991}. This is mainly done by selecting a certain ordering of the algebraic operations, 
...
communication-based operations \citep[pp.~210f.]{Ozsu1991}, whereas the predicted execution costs strongly depend on the costs for communication in the 
...
their schema or about cardinalities of intermediate results \citep[pp.214ff.]{Ozsu1991}, i.e. an estimation of the size of the according intermediate result 
...
$card(J)=card(R)*card(S)$ tuples (cf. \citet[p.~215]{Ozsu1991}). On the other side, the result set of the join could as well be empty when no tuples of both
