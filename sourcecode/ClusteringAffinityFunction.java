...
import separatetables.FragIDKey;
import separatetables.IllKey;
...
public class ClusteringAffinityFunction implements AffinityFunction, Serializable {
    ...
    /**
     * Concept Unique Identifiers (CUIs) of the disease terms (MeSH).
     */
    private ArrayList<String> cuis;
...
//##################### Constructors ######################
     ...
     */
    public ClusteringAffinityFunction() {
        this(DFLT_ALPHA);
    }
     ...
     */
    public ClusteringAffinityFunction(double alpha) {
        this.alpha = alpha;
     ...
     */
    public ClusteringAffinityFunction(double alpha, String termsFile, String similaritiesFile) {
        this.alpha = alpha;
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(similaritiesFile))) {
            while ((line = reader.readLine()) != null) {
                ...
                String[] things = line.split("<>");     // TODO get cui
...
//##################### Overwritten Methods ######################
    ...
    public int partition(Object key) {
        ...
        // If the key is of type InfoKey, then find the partition based on the derived fragmentation (contained in key)
        if (key instanceof IllKey) {
            IllKey illKey = (IllKey) key;
            partition = identifyCluster(illKey.getDisease());
        }
        else if (key instanceof FragIDKey) {
            partition = ((FragIDKey) key).getFragID();
        } else {
    ...
    public List<List<ClusterNode>> assignPartitions(AffinityFunctionContext affCtx) {
        if (affCtx == null)
            throw new IllegalArgumentException("AffinityFunctionContext passed to the clusteringbasedfragmentation.MyAffinityFunction's method " +
...
//##################### Clustering-based Fragmentation (Partition-Mappings, clusteringbasedfragmentation.Cluster-Algorithm)  ######################
    ...
    /**
     * This method calculates the clustering of the active domain of the relaxation
     ...
     * are the partitions). The resulting assignment is a list of clusters.
     * For details see {@link Cluster}
